...
public class GangTasks : IPlayerTaskGroup
{
    ...
    public GangTask RandomZoneTask(Gang gang, GangContact gangContact, TurfStatus gtStatus)
    {
        ...
        EntryPoint.WriteToConsole($"{gtStatus.ZoneName} GangTask: {gtStatus.AmbientEnemyGangs.Any()}, {gtStatus.EnemyGangsWithScenarios.Any()}, {gtStatus.HasRacketStores}, {options.Count}");
        if (options.Count > 0)
        {
            int rng = new Random().Next(options.Count);
            return options[rng]();
    ...
    public GangTask RandomUntrustedTask(Gang gang, GangContact gangContact) // tasks if you're not friendly yet i guess
    {
        ...
        TurfStatus gts = new TurfStatus(World, World.Zones, PlacesOfInterest, World.GangTerritories, Gangs, gangTerritory, gang);

        List<Func<GangTask>> options = new List<Func<GangTask>>{
            () => GangHit(gang, RandomKillRequirement, gangContact, enemyGang),
            () => GangAmbush(gang, gangContact, enemyGang,gts),
            () => GangArson(gang, gangContact),
            () => GangImpoundTheft(gang, gangContact)
        };
        int rng = new Random().Next(options.Count);
    ...
    public GangTask GangAmbush(Gang gang, GangContact gangContact, Gang targetGang, TurfStatus gangTerritoryStatus)
    {
        RivalGangAmbushTask newTask = new RivalGangAmbushTask(Player, Time, Gangs, PlacesOfInterest, ActiveDrops, Settings, World, Crimes, Weapons, Names, PedGroups, ShopMenus, ModItems, PlayerTasks, this, gangContact, gang, targetGang, GangTerritories, Zones, gangTerritoryStatus);
