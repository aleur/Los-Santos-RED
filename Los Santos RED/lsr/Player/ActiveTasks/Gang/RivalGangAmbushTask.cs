    ...
    public class RivalGangAmbushTask : GangTask, IPlayerTask
    {
        ...
        protected override void Loop()
        {
            while (true)
            {
                ...
                if (MembersKilled != 0)
                {
                    // If player has killed and there are no targets within x radius, mission failed.
                    if (!TargetSpawns.Any(ts => ts.CreatedPeople.Any(ped => ped.Pedestrian?.Exists() == true && ped.Pedestrian.IsValid() && !ped.IsDead && ped.Pedestrian.DistanceTo2D(Player.Character) <= 850f)))
                    {
                        break;
                    }
                    TargetSpawns.ForEach(x => x.CreatedPeople.Where(p => p.Pedestrian != null && p.Pedestrian.Exists() && !p.Pedestrian.IsPersistent).ToList().ForEach(m => { m.Pedestrian.IsPersistent = true; m.WasEverSetPersistent = true; }));
                }
        ...
        {
            if (TargetedScenario == null) return;

            // If player still in area
            TargetSpawns.ForEach(x => x.CreatedPeople.ForEach(m => m?.DeleteBlip()));
            TargetSpawns.ForEach(x => x.CreatedPeople.Where(p => p.Pedestrian != null && p.Pedestrian.Exists() && p.WasEverSetPersistent).ToList().ForEach(m => m.Pedestrian.IsPersistent = false));

            TargetedScenario.ArePedsTargeted = false;
        ...
        private void UpdateTargetSpawns()
        {
            ...
            if (TargetedScenario.PossibleGroupSpawns?.Any() == true)
            {
                TargetSpawns.AddRange(TargetedScenario.PossibleGroupSpawns
                        .Where(x => x.ArePedsTargeted && x.AttemptedSpawn)
                        .SelectMany(x => x.PossiblePedSpawns)
                        .Where(x => x.LocationSpawnTask != null)
                        .Select(x => x.LocationSpawnTask)
                );
                TargetSpawns.AddRange(TargetedScenario.PossibleGroupSpawns
                        .Where(x => x.ArePedsTargeted && x.AttemptedSpawn && x.PossibleVehicleSpawns?.Any(vs => !vs.IsEmpty) == true)
        ...
        {
            TargetedScenario.ArePedsTargeted = true;
            ...
            switch (AmbushTargetType)
            {
                ...
                case "Police":
                    ...
                    Replies = KillAllTargets ? new List<string>() {
                        ...
                        $"Found some {TargetAgency.ColorPrefix}{TargetAgency.ShortName}~s~ poking their noses where they don’t belong. Here’s their location. Deal with them fast. ~g~${PaymentAmount}~s~ waiting for you.",
                        $"Looks like {TargetAgency.ColorPrefix}{TargetAgency.ShortName}~s~ are snooping around. I’ve marked their position. Handle it quickly. ~g~${PaymentAmount}~s~ for the job.",
                        } :
                        new List<string>() {
                        ...
                        $"Found some {TargetAgency.ColorPrefix}{TargetAgency.ShortName}~s~ poking their noses where they don’t belong. Here’s their location. Need at least {KillRequirement} of them gone. ~g~${PaymentAmount}~s~ waiting for you.",
                        $"Looks like {TargetAgency.ColorPrefix}{TargetAgency.ShortName}~s~ are snooping around. Need at least {KillRequirement} dead. I’ve marked their position. Handle it quickly. ~g~${PaymentAmount}~s~ for the job.",
                        }
                    ; ;
                    break;
                case "Civilian":
