    ...
    public class GangRacketeeringTask : GangTask, IPlayerTask
    {
        ...
        private void GetRacketeeringSpots()
        {
            if (TargetZone != null)
            {
                ...
                EntryPoint.WriteToConsole($"RACKETEERING {HiringGang.ID}, {TargetZone.DisplayName} - {RacketeeringLocations.Count} LOCATIONS AVAILABLE, RANDOMIZING NOW");
                RacketeeringLocations = AvailableSpots.OrderBy(x => Guid.NewGuid()).Take(RandomItems.GetRandomNumberInt(1, 5)).ToList();
            }
        ...
        private void GetRandomRacketeeringSpots()
        {
            ...
            if (Zones.GetZone(selectedTerritory.ZoneInternalGameName) != null)
            {
                ...
                foreach (GameLocation possibleSpot in PossibleSpots)
                {
                    if (possibleSpot == null || possibleSpot.EntrancePosition == null)
                    {
                        continue;
                    }
