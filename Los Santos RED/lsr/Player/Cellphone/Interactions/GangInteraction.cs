...
=======
    ...
    private void AddGangAmbushSubMenu()
    {
        ...
        */
        ...
        TaskStartMenu.Activated += (sender, selectedItem) =>
        {
            GangTerritory gangTerritory = World.GangTerritories.GangTerritoriesList.Where(x => ActiveGang.EnemyGangs.Contains(x.GangID)).PickRandom();
            Gang gang = Gangs.GetGang(gangTerritory.GangID);
            TurfStatus gts = new TurfStatus(World, World.Zones, PlacesOfInterest, World.GangTerritories, Gangs, gangTerritory, gang);
            GangTask gt = Player.PlayerTasks.GangTasks.GangAmbush(ActiveGang, GangContact, TargetMenu.SelectedItem.Gang, gts);
            //gt.TargetZone = World.Zones.GetZone(ZonesMenu.SelectedItem.GangTerritory.ZoneInternalGameName);
            gt.Start();
            sender.Visible = false;
        };
        /*
