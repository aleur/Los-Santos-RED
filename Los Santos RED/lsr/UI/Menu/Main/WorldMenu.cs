...
public class WorldMenu : ModUIMenu
{
    ...
    public WorldMenu(MenuPool menuPool, UIMenu parentMenu, ISettingsProvideable settingsProvideable, ICrimes crimes, IIntoxicants intoxicants, IShopMenus shopMenus, UI ui)
    {
        MenuPool = menuPool;
    ...
    public void Setup()
    {
        WorldUIMenu = MenuPool.AddSubMenu(ParentMenu, "World Manager");
        ParentMenu.MenuItems[ParentMenu.MenuItems.Count() - 1].Description = "Change, Save, and Load Worlds.";
        ParentMenu.MenuItems[ParentMenu.MenuItems.Count() - 1].RightBadge = UIMenuItem.BadgeStyle.Crown;
        WorldUIMenu.SetBannerType(EntryPoint.LSRedColor);
        CreateWorldManagerMenu();
        WorldUIMenu.OnMenuOpen += WorldUIMenu_OnMenuOpen;
    }

    private void WorldUIMenu_OnMenuOpen(UIMenu sender)
    {
    ...
    {
        WorldUIMenu.Visible = false;
    }
    public override void Show()
    {
        Update();
        WorldUIMenu.Visible = true;
    }
    public override void Toggle()
    {
        Update();
        if (!WorldUIMenu.Visible)
        {
            WorldUIMenu.Visible = true;
        }
        else
    ...
    private void CreateWorldManagerMenu()
    {
        WorldUIMenu.Clear();
        /*
        ...
        WorldUIMenu.AddItem(ShowWorldSaveMenu);*/

        UIMenuItem ShowWorldTemplateMenu = new UIMenuItem("Templates", "Shows a list of world templates.");
        ShowWorldTemplateMenu.RightBadge = UIMenuItem.BadgeStyle.Makeup;
        ShowWorldTemplateMenu.Activated += (s, e) =>
        {
            UI.TemplatePauseMenu.Toggle();
            WorldUIMenu.Visible = false;
        };
        WorldUIMenu.AddItem(ShowWorldTemplateMenu);

        UIMenu worldSubMenu = MenuPool.AddSubMenu(WorldUIMenu, "Current World"); // if no world loaded in, grey out option, and have bottom text read, modify current world
        ...
        WorldUIMenu.OnItemSelect += OnItemSelect;
        WorldUIMenu.OnListChange += OnListChange;
    }
    ...
    {
        UIMenu genericSubMenu = MenuPool.AddSubMenu(WorldUIMenu, name);
