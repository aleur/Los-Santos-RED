...
public abstract class SpawnTask
{
    ...
    protected virtual void AttemptVehicleSpawn()
    {
        ...
        LastCreatedVehicle.IsTheftTarget = IsTheftTarget;
        if (HasPersonToSpawn)
        {
            if (WillAddDriver)
            {
    ...
    {
        PedExt Person = CreatePerson(-1);
        if (Person != null && Person.Pedestrian.Exists()) Person.IsTargetedByPlayer = IsAmbushTarget;
        if (!AllowBuddySpawn)
        {
            return;
        }
        ...
        for (int BuddyIndex = 1; BuddyIndex <= BuddiesToSpawn; BuddyIndex++)
        {
            ...
            if (PersonType != null)
            {
                ...
                if (p != null && p.Pedestrian.Exists()) p.IsTargetedByPlayer = IsAmbushTarget;
            }
        }    
    }
