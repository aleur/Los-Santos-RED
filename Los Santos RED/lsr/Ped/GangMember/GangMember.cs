...
public class GangMember : PedExt, IWeaponIssuable
{
    ...
    public void SetStats(DispatchablePerson dispatchablePerson, IShopMenus shopMenus, IWeapons weapons, bool addBlip, bool forceMelee, bool forceSidearm, bool forceLongGun, Zone zone, List<GangTerritory> gangTerritoryList)
    {
        if (!Pedestrian.Exists())
        {
            return;
        ...
        bool isGangTerritoryListNull = gangTerritoryList == null;
        GangTerritory gt = isGangTerritoryListNull ? null : gangTerritoryList.FirstOrDefault(x => x.ZoneInternalGameName.Equals(zone.InternalGameName));

        if (isGangTerritoryListNull || gt == null)
        {
            ...
            WillDealDrugs = RandomItems.RandomPercent(Gang.DrugDealerPercentage);
            WillHaveLongGuns = RandomItems.RandomPercent(Gang.PercentageWithLongGuns);
            WillHaveSidearms = RandomItems.RandomPercent(Gang.PercentageWithSidearms);
            WillHaveMelee = RandomItems.RandomPercent(Gang.PercentageWithMelee);
        }
        else
        {
            WillFight = RandomItems.RandomPercent(gt.FightPercentage);
            WillFightPolice = RandomItems.RandomPercent(gt.FightPolicePercentage);
            WillAlwaysFightPolice = RandomItems.RandomPercent(gt.AlwaysFightPolicePercentage);
            WillDealDrugs = RandomItems.RandomPercent(gt.DrugDealerPercentage);
            WillHaveLongGuns = RandomItems.RandomPercent(gt.PercentageWithLongGuns);
            WillHaveSidearms = RandomItems.RandomPercent(gt.PercentageWithSidearms);
            WillHaveMelee = RandomItems.RandomPercent(gt.PercentageWithMelee);
        }
